...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [self.bigCollect setContentOffset:CGPointMake((self.clickNum) * (DeviceWidth + margin), 0)];
}

-(void)viewDidLoad{
    [super viewDidLoad];
    self.view.backgroundColor = ColorWhite;
    UIBarButtonItem  *backItem = [[UIBarButtonItem alloc] initWithTitle:@"back" style:0 target:self action:@selector(backBaseControl)];
...
-(void)configBigPhotoControllerUI{
    ...
    _bigCollect = [[UICollectionView alloc] initWithFrame:CGRectMake(-margin/2, 0, DeviceWidth + margin, DeviceHeight - 64) collectionViewLayout:flowLayout];
    ...
    _bigCollect.backgroundColor = [UIColor whiteColor];
...
//            }
//        }
        [_ChooseArr enumerateObjectsUsingBlock:^(FZJPhotoModel *obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if([obj.imageName isEqualToString:[_fetchResult[select.index] valueForKey:@"filename"]])
            {
                *stop = YES;//停止遍历数组
                if(*stop)
                {
                    [_ChooseArr removeObject:obj];//删除图片哈
                }
            }
        }];

    }
    self.returnBlock(_ChooseArr);
}
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    if (scrollView == (UIScrollView *)_bigCollect) {//UICollectionView是继承于UIScrollView的
        CGFloat current = scrollView.contentOffset.x / (DeviceWidth + margin) + 1;
