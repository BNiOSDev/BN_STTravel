...
#define SPKOPENIMCHATID @"spkOpenChatId"
...
- (void)callThisInDidFinishLaunching
{
    ...
    if ([self exampleInit]) {
        ...
        NSString *chatId = [[NSUserDefaults standardUserDefaults] objectForKey:SPKOPENIMCHATID];
        if ([chatId length]) {
            [self exampleLoginWithUserID:chatId password:chatId successBlock:nil failedBlock:nil];
        }
...
//#warning TODO: CHANGE TO YOUR AppKey
        /// 线上环境，更换成你自己的AppKey
        [[YWAPI sharedInstance] syncInitWithOwnAppKey:@"23558595" getError:&error];
    } else {
...
#warning JUST COMMENT OUT THIS FUNCTION IF YOU DO NOT NEED THE CUSTOM CONVERSATION ON THE TOP
            ...
            [[NSUserDefaults standardUserDefaults] setObject:aUserID forKey:SPKOPENIMCHATID];
        ...
        if ( conversation != nil ) {
            ...
            if ( [aViewController isKindOfClass:[UINavigationController class]] ) {
                ...
                if ( aViewController.navigationController ) {
                    ...
                    UIBarButtonItem *rightBarButtonItem  = [[UIBarButtonItem alloc] initWithTitle:@"关闭" style:UIBarButtonItemStylePlain target:nil action:nil];
